...
export default function MobileVisualization({
  ...
  return (
    <Card className="col-span-1 w-full shadow-sm border border-gray-200">
      <CardHeader className="p-3 sm:p-4 bg-white border-b border-gray-200">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 bg-gray-100">
            <TabsTrigger value="heart" className="flex items-center gap-2 data-[state=active]:bg-white">
              <Heart className="h-4 w-4 text-pink-500" /> Corazón
            </TabsTrigger>
            <TabsTrigger value="lungs" className="flex items-center gap-2 data-[state=active]:bg-white">
      ...
      <CardContent className="p-0 sm:p-0">
        <TabsContent value="heart" className="h-[200px] sm:h-[260px] mt-0">
          <HeartAnimation rhythmType={rhythmType} isPlaying={isPlaying} />
        </TabsContent>
        <TabsContent value="lungs" className="h-[200px] sm:h-[260px] mt-0">
